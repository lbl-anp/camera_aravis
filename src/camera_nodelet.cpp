...
NODEEX CameraNodelet::GetGcFirstChild(ArvGc *pGenicam, NODEEX nodeex)
{
    ...
    ROS_DEBUG("GFC name=%s, node=%p, sib=%p", szName, nodeex.pNode, nodeex.pNodeSibling);
...
NODEEX CameraNodelet::GetGcNextSibling(ArvGc *pGenicam, NODEEX nodeex)
{
    ...
    ROS_DEBUG("GNS name=%s, node=%p, sib=%p", nodeex.szName, nodeex.pNode, nodeex.pNodeSibling);
...
void CameraNodelet::onInitImpl()
{
  ...
  idSoftwareTriggerTimer = 0;
  ros::Duration retry_delay(1.0);

  // TODO: how did this work with multiple devices?
  // Connect to a camera and retry if none detected
  int num_devices = 0;
  while (TRUE)
  {
    arv_update_device_list();
    int interfaces = arv_get_n_interfaces();
    num_devices = arv_get_n_devices();

    if (num_devices > 0)
    {
      // Print out some useful info.
      ROS_INFO("Attached cameras:");
      ROS_INFO("# Interfaces: %d", interfaces);
      ROS_INFO("# Devices: %d", num_devices);
    ...
    ROS_WARN_ONCE("No aravis cameras detected, will continue to retry...");
    retry_delay.sleep();
    ros::spinOnce();
  }
  ...
  while (TRUE)
  {
    ...
    {
      ROS_WARN_ONCE("Could not open camera %s, will continue to retry...", pszGuid);
      retry_delay.sleep();
      ros::spinOnce();
    }
  }
